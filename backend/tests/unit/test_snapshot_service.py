...
class TestSnapshotService:
    ...
    async def test_snapshot_comparison_generates_diff(
        ...
        assert "packages_changed" in diff or "services_changed" in diff
    ...
    async def test_rollback_initiation(
        ...
        assert rollback.status == RollbackStatus.INITIATED
        assert rollback.reason == "Testing rollback"
    
    async def test_rollback_validates_snapshot_type(
        self, test_session, test_tenant, create_test_bundle, create_test_asset
    ):
        """Test that rollback validates snapshot is pre-patch"""
        service = SnapshotService()
        bundle = await create_test_bundle(tenant_id=str(test_tenant.id))
        asset = await create_test_asset(tenant_id=str(test_tenant.id))
        
        # Create POST-patch snapshot (invalid for rollback)
        snapshot = await service.capture_snapshot(
            db=test_session,
            bundle_id=bundle.id,
            asset_id=asset.id,
        ...
        # Try to rollback to post-patch snapshot (should fail)
        with pytest.raises(ValueError, match="pre-patch|PRE_PATCH"):
    ...
    async def test_snapshot_integrity_validation(
        ...
        # Verify integrity
        is_valid = await service.verify_snapshot_integrity(
            db=test_session,
            snapshot_id=snapshot.id
        )
    ...
    async def test_snapshot_integrity_detects_corruption(
        ...
        snapshot.system_state["packages"] = []
